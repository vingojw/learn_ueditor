    ...
    function insertSingle(){
        ...
        if(/^<iframe/.test(url)){
            ...
            if(width.value){
                ...
                //如果是复制的通用代码里面的，那么取里面的值
                var uWidth = url.match(/width=(\d+)/);
                var uHeight = url.match(/height=(\d+)/);
                if(uWidth){
                    newIframe.setAttribute("width",uWidth[1]);
                    newIframe.setAttribute("height",uHeight[1]);
                }
            }
